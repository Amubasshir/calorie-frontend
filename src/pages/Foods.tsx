...
const Foods: React.FC = () => {
  ...
  const [foodAllCategories, setFoodAllCategories] = useState([]);
      ...
      async function getSingleFood() {
        try {
          const foodResult = await foodService.getAllFoodCategories();
          // setUser(user);
          setFoodAllCategories(foodResult);
        } catch (error) {
          // setUser(null);
        } finally {
          // setLoading(false);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="space-y-6">
        {foodAllCategories.map((category, i) => (
          <div key={i} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div className="p-6">
              {/* Mobile: Only show category name and link */}
              <div className="md:hidden">
                <Link
                  to={`/alimenti/${category.category}`}
              ...
              <div className="hidden md:block">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {category.subcategories.map((subCategory, index) => (
                    <Link
                      key={index}
                      to={`/alimenti/${i}/${subCategory?.toLowerCase().replace(/\s+/g, '-')}`}
